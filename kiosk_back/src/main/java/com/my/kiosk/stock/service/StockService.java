...
public class StockService {
	...
	public List<StockfrontDTO> getStockfrontList(int place_id) {
		List<Stock> s=stockmapper.findByPlaecID(place_id);
		List<StockfrontDTO> sf = new ArrayList<>();
		 for(Stock ss : s){
			StockfrontDTO sfs=new StockfrontDTO();
			sfs.setMenu_id(ss.getMenu_id());
			sfs.setStock_qty(ss.getStock_qty());
			Menu menu = stockmapper.findMenuById(ss.getMenu_id());
			if (menu != null) {
				sfs.setName(menu.getName());
				sfs.setImgpath(menu.getImg());
			}
				sf.add(sfs);
		 }
		return sf;
	...
	public Menu getMenuById(int id){
		return stockmapper.findMenuById(id);
	}

	public void productSell(int menu_id, int place_id, int amount){
		Stock stock = stockmapper.getStock(menu_id, place_id);
		System.out.println(amount);
		stock.setStock_qty(stock.getStock_qty() - amount);
		
		stockmapper.productSell(stock);
